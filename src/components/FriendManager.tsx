...
export const FriendManager: React.FC<FriendManagerProps> = ({ currentUser }) => {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm space-y-6">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-stone-100">
        <div>
          <h3 className="font-serif font-bold text-base text-stone-800 flex items-center gap-2">
            <Users className="w-5 h-5 text-amber-600" /> 友達・メンバー連携＆カテゴリ設定
          </h3>
      ...
      <div className="bg-stone-50 rounded-xl p-3.5 border border-stone-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2 text-stone-700">
          <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" />
          <span>あなたのマイID (友達に伝えて検索してもらえます):</span>
        </div>
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <code className="bg-white px-2.5 py-1 rounded-lg border border-stone-300 text-[11px] font-mono text-stone-800 select-all">
            {currentUser.uid}
          </code>
          <button
            type="button"
            onClick={handleCopyUid}
            className="flex items-center gap-1 bg-white hover:bg-stone-100 text-stone-700 px-2.5 py-1 rounded-lg border border-stone-300 transition-colors font-medium shrink-0 cursor-pointer"
          >
            {copiedUid ? (
              <>
      ...
      <div className="bg-gradient-to-br from-amber-500/10 via-amber-50/60 to-stone-50 rounded-2xl p-4 border border-amber-200/80 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Share2 className="w-4 h-4 text-amber-700" />
            <h4 className="text-xs font-bold text-amber-950">友達招待リンク ＆ QRコード</h4>
          </div>
          <button
            type="button"
            onClick={() => setShowQrCode(!showQrCode)}
            className="flex items-center gap-1 text-[11px] font-bold text-amber-800 bg-amber-100 hover:bg-amber-200 px-3 py-1 rounded-xl transition-all cursor-pointer border border-amber-300"
          >
        ...
        <div className="flex items-center gap-2 bg-white p-2 rounded-xl border border-amber-200">
          <Link className="w-3.5 h-3.5 text-amber-600 shrink-0 ml-1" />
          <input
            type="text"
            readOnly
            value={inviteUrl}
            className="text-[11px] text-stone-700 bg-transparent w-full focus:outline-hidden font-mono truncate select-all"
          />
          <button
            type="button"
            onClick={handleCopyInviteLink}
            className="flex items-center gap-1 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs px-3 py-1.5 rounded-lg shadow-2xs transition-all shrink-0 cursor-pointer"
          >
            {copiedInviteLink ? (
              <>
        ...
        {showQrCode && (
          <div className="pt-3 border-t border-amber-200/80 flex flex-col items-center justify-center space-y-3 bg-white p-4 rounded-xl border border-stone-200 shadow-2xs animate-fade-in">
            ...
            <div className="p-3 bg-white rounded-2xl border-2 border-amber-400 shadow-sm relative">
              <img
                src={qrCodeImageUrl}
                alt={`${currentUser.displayName}の招待QRコード`}
                className="w-44 h-44 object-contain rounded-lg"
              />
              <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-amber-600 text-white text-[9px] font-bold px-2 py-0.5 rounded-full shadow-2xs whitespace-nowrap">
                LifeLog AI Diary
              </div>
            </div>
      ...
      {statusMessage && (
        <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-xs font-bold text-emerald-700 text-center animate-fade-in">
          {statusMessage}
        </div>
      )}
      ...
      <div className="space-y-3 pt-1">
        <h4 className="text-xs font-bold text-stone-800 flex items-center gap-1.5">
          <UserPlus className="w-4 h-4 text-amber-600" /> 友達を探して追加する
        </h4>
        ...
        <form onSubmit={handleSearchUsers} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 rounded-xl border border-stone-300 text-xs text-stone-800 focus:outline-hidden focus:ring-2 focus:ring-amber-500 bg-stone-50"
            />
          </div>
          <button
            type="submit"
            disabled={isSearching || !searchQuery.trim()}
            className="bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs px-4 py-2 rounded-xl transition-all cursor-pointer disabled:opacity-50 shrink-0"
          >
            検索
          </button>
        </form>
        ...
        <div className="flex items-center gap-2 pt-1">
          <span className="text-[11px] text-stone-400 font-medium">クイックお試し追加:</span>
          <button
            type="button"
            onClick={() => handleAddDemoFriend('山田 太郎 (家族)', 'Default')}
            className="text-[11px] bg-amber-50 hover:bg-amber-100 text-amber-800 font-semibold px-2.5 py-1 rounded-lg border border-amber-200 transition-colors cursor-pointer"
          >
            ＋ 山田さん追加
          </button>
          <button
            ...
            className="text-[11px] bg-stone-100 hover:bg-stone-200 text-stone-700 font-semibold px-2.5 py-1 rounded-lg border border-stone-300 transition-colors cursor-pointer"
          >
            ＋ 佐藤さん追加
          </button>
        </div>
        ...
        {searchResults.length > 0 && (
          <div className="bg-amber-50/50 rounded-xl p-3 border border-amber-200/80 space-y-2">
            <span className="text-[11px] font-bold text-amber-900 block">検索結果:</span>
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <img
                        ...
                        className="w-7 h-7 rounded-lg object-cover bg-amber-100"
                        referrerPolicy="no-referrer"
                      />
                      <div>
                        <span className="font-bold text-stone-800 block">{userRes.displayName}</span>
                        <span className="text-[10px] text-stone-500">{userRes.bio || '自己紹介未設定'}</span>
                      </div>
                    </div>
                    ...
                    {isAlreadyFriend ? (
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs px-3 py-1.5 rounded-lg shadow-2xs transition-all cursor-pointer"
                      >
      ...
      <div className="pt-2 border-t border-stone-100 space-y-3">
        ...
        {loading ? (
          <p className="text-xs text-stone-500 text-center py-4">読み込み中...</p>
        ) : friends.length === 0 ? (
          <div className="bg-stone-50 rounded-xl p-6 text-center border border-dashed border-stone-200 space-y-2">
            <Users className="w-8 h-8 text-stone-400 mx-auto" />
            <p className="text-xs text-stone-600 font-medium">友達がまだ登録されていません</p>
          ...
          <div className="space-y-3">
            {friends.map((friend) => {
              ...
              return (
                <div
                  key={friend.id}
                  className="bg-stone-50/80 rounded-2xl p-4 border border-stone-200/90 space-y-3 transition-all hover:bg-stone-50"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <img
                        ...
                        className="w-9 h-9 rounded-xl object-cover bg-amber-100 border border-amber-200"
                        referrerPolicy="no-referrer"
                      />
                      <div>
                      ...
                      {allCategories.map((cat) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs transition-all border cursor-pointer ${
                              isChecked
