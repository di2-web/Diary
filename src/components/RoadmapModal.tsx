...
export const RoadmapModal: React.FC<RoadmapModalProps> = ({ isOpen, onClose }) => {
  ...
  const steps: StepItem[] = [
    ...
    {
      id: 'phase-2',
      title: 'Phase 2: 気分の波 (Wave Canvas) ＆ 重み付け連携 (完了)',
      description: '24時間の気分の高低を描画し、AIがどの出来事を重視するか判定',
      status: 'completed',
      details: [
        '✅ 24時間軸の気分の波ドラッグ描画UI (Wave Canvas)',
        '✅ 気分の波データとAI生成プロンプトの連携（感情の山場をエピソード化）',
        '✅ 時間軸上の投稿素材ピンマッピング＆プレビュー',
        '✅ クイック波型プリセット（穏やか、昼に盛り上がり、波乱万丈、夜型）',
      ],
    },
    {
      id: 'phase-3',
      title: 'Phase 3: 手帳デコレーション ＆ リッチ装飾 (完了)',
      description: 'AIが作った日記を自分で自由にデコる楽しさの提供',
      status: 'completed',
      details: [
        '✅ 手帳風デザインキャンバス（和紙・クラフト紙・方眼・ドット・カフェレトロ）',
        '✅ スタンプ・シールペタペタ機能（感情スタンプ、シールを自由配置＆削除）',
        '✅ 手帳風フォント切り替え（明朝体・手書き風・ゴシック・タイプライター）',
        '✅ 日記本文の直感的な手動リライト・修正編集保存機能',
      ],
    },
    {
      id: 'phase-4',
      title: 'Phase 4: 日常共有SNS ＆ カテゴリ交流 (完了)',
    ...
    {
      id: 'phase-5',
      title: 'Phase 5: カレンダー振り返り・検索 ＆ 自動データ管理 (完了)',
      description: '要件定義書⑤に対応：過去思い出の瞬時検索と未保護素材の自動クリーンアップ',
      status: 'completed',
      details: [
        '✅ 月間カレンダー・キーワード全文検索機能（タイトル・本文・日付を瞬時検索）',
        '✅ 自動消去（パージ）機能：保護（ピン留め）されていない素材の一括安全整理',
        '✅ セキュリティ＆アクセス制御（Row Level Security / Firestoreルール最適化）',
        '✅ 感情ログ・思い出のキーワードマイページ検索＆バックアップ保護',
      ],
    },
  ];
